...
class deliveryorder extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $data['link'] = site_url('/') . "transaksi/deliveryorder/list_data";
    ...
    public function edit()
    {
        ...
        $data['dopaket'] 			  = $this->orm->dopaket->where('iddeliveryorder', $iddeliveryorder)->order('iddeliveryorder DESC');
    ...
    public function save()
    {
        ...
        foreach ($idPackage as $key => $row) {
            $dopkt['idpackaging']     = $idPackage[$key];
            $dopkt['iddeliveryorder'] = $iddeliveryorder;
            $this->orm->dopaket->insert($dopkt);
        }
    ...
    public function update()
    {
        ...
        $data['idrefstore'] = $_SESSION['user']['idrefstore'];
        $data['flag_app']   = "WEB";
        //delete dulu yanglama
        $this->orm->dopaket->where('iddeliveryorder', $data['iddeliveryorder'])->delete();
    ...
    public function delete()
    {
        ...
        if ($iddeliveryorder != null) {
            ...
            if ($ress) {
                echo json_encode(array(
                    'response' => 'success',
                    'msg' => 'Delete Data DO Berhasil'
                ));
            } else {
                echo json_encode(array(
                    'response' => 'failed',
                    'msg' => 'Delete Data DO Gagal'
                ));
            }
        }
    }
...
<table width='100%'>" . "<tr>" . '<td style="text-align: center;"></td>' . '<td></td>' . '<td style="text-align: center;"></td>' . '<td></td>' . '<td width="120" style="text-align: center;"></td>' . '<td></td>' . '<td style="text-align: center;"></td>' . '<td></td>' . '</tr>';
        $html .= '<tr>' . '<td style="text-align: left;"><br/><br/><br/><br/></td>' . '<td></td>' . '<td style="text-align: left;"><br/><br/><br/><br/></td>' . '<td></td>' . '<td width="120" style="text-align: left;"><br/><br/><br/><br/></td>' . '<td></td>' . '<td  style="text-align: left;"><br/><br/><br/><br/></td>' . '<td></td>' . '</tr>';
        $html .= '<tr>' . '<td style="text-align: left;"></td>' . '<td></td>' . '<td style="text-align: left;"></td>' . '<td></td>' . '<td width="110" style="text-align: left;"><b><u>' . $this->disetujui . '</u></b></td>' . '<td></td>' . '<td width="240" style="text-align: left;"><br/>  <b><u>' . $this->oleh . '</u></b> </td>' . '<td></td>' . '</tr>' . '</table>';
