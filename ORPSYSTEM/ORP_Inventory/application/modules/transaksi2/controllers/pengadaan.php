 ...
 Account::_valLogin();
    ...
    public function cetakDaftar($idrefstore = null) {
        ...
        if ($idrefstore == null) {
            ...
            $data = $this->orm->pengadaan->where('idrefstore', $_SESSION['user']['idrefstore'])->order('tanggalPengadaan DESC');
            $email = $_SESSION['user']['email'];
        }//from mobile
        else{
            $store = $this->orm->refstore->where('idrefstore', $idrefstore)->fetch();
            $data = $this->orm->pengadaan->where('idrefstore', $idrefstore)->order('tanggalPengadaan DESC');
