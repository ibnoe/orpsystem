<div class="modal-body">
							...
							<table width="400px">
								<tr>
									<td id="title">
										...
										<a href="#" class="btn btn-danger" id="deleteRowPO33"><i class="fa fa-minus"></i>Hapus</a> 
									</td>
								</tr>
								<hr/>
								<tr width="">
									<td>
										...
										<table id="pemesanan_paket" width="">
											...
											<?php 
												$j =1;
												foreach ($dopaket as $row3) {?>
											<tr id="ed_tr2_<?php echo $j?>">
												<td width="25px">
													<input type="checkbox" size="1" name="edchk2" value="<?php echo $j?>">
												</td>
												<td width="40px">
													<input  type="text" size="3" id="pemesanan_paket_1"  name="pemesanan_paket[]" value="<?php echo $j?>"/>
												</td>
												<td width="150px">
													<select  id="idpackage_<?php echo $j?>"  name="idpackage[]"  class="selectpicker show-menu-arrow" data-live-search="true" data-width="450px">
														<option value="0">Pilih Paket</option>;
														<?php
														foreach ($this->orm->package->order('idpackage ASC') as $row) {?>
															<option value="<?php echo $row['idpackage']?>" <?php echo $row['idpackage']==$row3['idpackaging'] ? 'selected' : '';?>><?php echo $row['namepackage']?></option>
														<?php }?>
													</select>
												</td>
											</tr>
											<script>
												...
												$("#idpackage_<?php echo $j?>").selectpicker ({});   
	...
	//});
		...
		$("#addRowPO33").click(function(){
			htm = '<tr id="ed_tr_'+j+'">'+
						'<td width="25px">'+
							'<input type="checkbox" size="1" name="edchk2" value="'+j+'">'+
						'</td>'+
						'<td width="40px">'+
		...
		$("#deleteRowPO33").click(function(){
			//alert('s');
			var selectedid2 = new Array();
			$('input[name="edchk2"]:checked').each(function() {
				selectedid2.push(this.value);
			});
			alert(selectedid2);
			for ( var i = 0, l = selectedid2.length; i < l; i++ ) {
				$('#ed_tr2_'+selectedid2[ i ]).remove();
			}
		});
	});
